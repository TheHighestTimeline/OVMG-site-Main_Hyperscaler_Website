...
import { PLOT, ROAD, SIDEWALKS, CROSSWALK, MARKET, COMMUNITY_CENTER, FOOTPRINTS } from '../sceneLayout';
...
export function createBaseWorld(): THREE.Group {
  ...
  // Manhole
  const manhole = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 0.01, 20), M.paintedMetal);
  ...
  const drain = box(0.5, 0.012, 0.22, M.paintedMetal, 'Drain');
