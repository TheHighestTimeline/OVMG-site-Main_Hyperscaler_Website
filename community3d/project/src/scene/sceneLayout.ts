...
/** Lamp X positions chosen from measured screen projections so no lifted lamp
 *  crosses either facade: safe screen slots are the facade gap (45-55%) and
 *  right of the market (>65%). See docs/PROGRESS.md QA round 4. */
export const LAMPS: Placement[] = [
  { position: { x: -5.55, y: 0, z: 0.98 }, rotationY: 0 },        // screen ~67% (right of market)
  { position: { x: -2.60, y: 0, z: -2.30 }, rotationY: Math.PI }, // screen ~70% (right of market)
  { position: { x: -1.20, y: 0, z: 0.98 }, rotationY: 0 },        // screen ~51% (facade gap, rear walk)
  { position: { x: 3.20, y: 0, z: -2.30 }, rotationY: Math.PI },  // screen ~47% (facade gap, front walk)
];
...
export const CAMERA = {
  ...
  mobile:  { position: { x: 11.5, y: 9.0, z: -10.5 }, target: { x: 0, y: 1.7, z: 0 }, viewHeight: 10.8 },
