...
export function mountCity3D(opts: MountOptions): City3D {
  ...
  // static warm pools under each lamp head (never lift with the lamp)
  LAMPS.forEach((lp, i) => {
    const pool = new THREE.Mesh(new THREE.CircleGeometry(0.85, 20), M.lightPool);
